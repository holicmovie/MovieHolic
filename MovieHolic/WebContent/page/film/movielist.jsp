<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@ include file="/template/header.jsp"%>
<%@ include file="/template/nav_style.jsp"%>
<%@ include file="/template/boot_431.jsp"%>
</head>
<body class="left-sidebar is-preload"  style="background-color: black;">
<div id="page-wrapper">

<!-- Header -->
	<div id="header" style="background-image: none; padding:4em 0 0 0;"">
<%@ include file="/template/nav.jsp"%>
	</div>
<!-- boxoffice -->
<section class="carousel" style="background-color: rgb(3,39,49);">
	<div class="reel">
		<h2>
			<strong>Best List</strong>
		</h2><br>
		<article>
			<a href="/MovieHolic/page/film/moviedetail.jsp"
				class="image featured"><img
				src="/MovieHolic/images/avengers4.jpg" alt=""></a>
			<header>
				<h6>
					키트리추천영화<br>
					<a href="/MovieHolic/page/film/moviedetail.jsp">ID : damm</a>
				</h6>
			</header>
		</article>

		<article>
			<a href="#" class="image featured"><img
				src="/MovieHolic/images/brother.jpg" alt="" /></a>
			<header>
				<h6>
					키트리추천영화<br>
					<a href="/MovieHolic/page/film/moviedetail.jsp">ID : shz</a>
				</h6>
			</header>
		</article>

		<article>
			<a href="#" class="image featured"><img
				src="/MovieHolic/images/capma.jpg" alt="" /></a>
			<header>
				<h6>
					키트리추천영화<br>
					<a href="/MovieHolic/page/film/moviedetail.jsp">ID : beom-jun</a>
				</h6>
			</header>
		</article>

		<article>
			<a href="#" class="image featured"><img
				src="/MovieHolic/images/watching.jpg" alt="" /></a>
			<header>
				<h6>
					키트리추천영화<br>
					<a href="/MovieHolic/page/film/moviedetail.jsp">ID : ant</a>
				</h6>
			</header>
		</article>

		<article>
			<a href="#" class="image featured"><img
				src="/MovieHolic/images/money.jpg" alt="" /></a>
			<header>
				<h6>
					키트리추천영화<br>
					<a href="/MovieHolic/page/film/moviedetail.jsp">ID : reasult</a>
				</h6>
			</header>
		</article>

		<article>
			<a href="#" class="image featured"><img
				src="/MovieHolic/images/pic01.jpg" alt="" /></a>
			<header>
				<h6>
					키트리추천영화 <br> <a href="/MovieHolic/page/film/moviedetail.jsp">ID
						: gwangqq</a>
				</h6>
			</header>
		</article>
	</div>
</section>

<!-- good rank list -->
<section class="carousel">
	<div class="reel" id="romance">
		<h2>
			<strong>GOOD Ranking</strong>
		</h2><br>
		<article>
			<a href="#" class="image featured"><img
				src="/MovieHolic/images/brother.jpg" alt="" /></a>
			<header>
				<h6>
					키트리추천영화<br>
					<a href="/MovieHolic/page/film/moviedetail.jsp">ID : reasult</a><br>
					좋아요 : <a href="/MovieHolic/page/film/moviedetail.jsp">216842 </a>
					싫어요 : <a>3</a>
				</h6>
			</header>
		</article>
		<article>
			<a href="#" class="image featured"><img
				src="/MovieHolic/images/avengers4.jpg" alt="" /></a>
			<header>
				<h6>
					키트리추천영화<br>
					<a href="/MovieHolic/page/film/moviedetail.jsp">ID : reasult</a><br>
					좋아요 : <a href="/MovieHolic/page/film/moviedetail.jsp">16842 </a>
					싫어요 : <a>3</a>
				</h6>
			</header>
		</article>

		<article>
			<a href="#" class="image featured"><img
				src="/MovieHolic/images/capma.jpg" alt="" /></a>
			<header>
				<h6>
					키트리추천영화<br>
					<a href="/MovieHolic/page/film/moviedetail.jsp">ID : reasult</a><br>
					좋아요 : <a href="/MovieHolic/page/film/moviedetail.jsp">6342 </a> 싫어요
					: <a>3</a>
				</h6>
			</header>
		</article>

		<article>
			<a href="#" class="image featured"><img
				src="/MovieHolic/images/watching.jpg" alt="" /></a>
			<header>
				<h6>
					키트리추천영화<br>
					<a href="/MovieHolic/page/film/moviedetail.jsp">ID : reasult</a><br>
					좋아요 : <a href="/MovieHolic/page/film/moviedetail.jsp">4842 </a> 싫어요
					: <a>3</a>
				</h6>
			</header>
		</article>

		<article>
			<a href="#" class="image featured"><img
				src="/MovieHolic/images/money.jpg" alt="" /></a>
			<header>
				<h6>
					키트리추천영화<br>
					<a href="/MovieHolic/page/film/moviedetail.jsp">ID : reasult</a><br>
					좋아요 : <a href="/MovieHolic/page/film/moviedetail.jsp">842 </a> 싫어요
					: <a>3</a>
				</h6>
			</header>
		</article>

		<article>
			<a href="#" class="image featured"><img
				src="/MovieHolic/images/pic04.jpg" alt="" /></a>
			<header>
				<h6>
					키트리추천영화<br>
					<a href="/MovieHolic/page/film/moviedetail.jsp">ID : reasult</a><br>
					좋아요 : <a href="/MovieHolic/page/film/moviedetail.jsp">42 </a> 싫어요 :
					<a>3</a>
				</h6>
			</header>
		</article>


	</div>


</section>


<!-- 최신 유저 리스트 -->

<!-- user movie list -->
<section class="carousel" style="padding:1.5em 0 0 0;margin:0 0 0 0;">
	<div class="reel">
		<h2><strong>New list</strong></h2>
		<article>
			<a href="#" class="image featured"><img
				src="/MovieHolic/images/brother.jpg" alt="" /></a>
			<header>
				<h6>
					키트리추천영화<br>
					<a href="/MovieHolic/page/film/moviedetail.jsp">ID : asdf</a>
				</h6>
			</header>
		</article>

		<article>
			<a href="#" class="image featured"><img
				src="/MovieHolic/images/avengers4.jpg" alt="" /></a>
			<header>
				<h6>
					키트리추천영화<br>
					<a href="/MovieHolic/page/film/moviedetail.jsp">ID : 12ras</a>
				</h6>
			</header>
		</article>

		<article>
			<a href="#" class="image featured"><img
				src="/MovieHolic/images/capma.jpg" alt="" /></a>
			<header>
				<h6>
					키트리추천영화<br>
					<a href="/MovieHolic/page/film/moviedetail.jsp">ID : sde111</a>
				</h6>
			</header>
		</article>

		<article>
			<a href="#" class="image featured"><img
				src="/MovieHolic/images/watching.jpg" alt="" /></a>
			<header>
				<h6>
					키트리추천영화<br>
					<a href="/MovieHolic/page/film/moviedetail.jsp">ID : monaque</a>
				</h6>
			</header>
		</article>

		<article>
			<a href="#" class="image featured"><img
				src="/MovieHolic/images/money.jpg" alt="" /></a>
			<header>
				<h6>
					키트리추천영화<br>
					<a href="/MovieHolic/page/film/moviedetail.jsp">ID : damm</a>
				</h6>
			</header>
		</article>

		<article>
			<a href="#" class="image featured"><img
				src="/MovieHolic/images/pic01.jpg" alt="" /></a>
			<header>
				<h6>
					키트리추천영화<br>
					<a href="/MovieHolic/page/film/moviedetail.jsp">ID : damm</a>
				</h6>
			</header>
		</article>

		<article>
			<a href="#" class="image featured"><img
				src="/MovieHolic/images/pic02.jpg" alt="" /></a>
			<header>
				<h6>
					키트리추천영화<br>
					<a href="/MovieHolic/page/film/moviedetail.jsp">ID : damm</a>
				</h6>
			</header>
		</article>

		<article>
			<a href="#" class="image featured"><img
				src="/MovieHolic/images/pic03.jpg" alt="" /></a>
			<header>
				<h6>
					키트리추천영화<br>
					<a href="/MovieHolic/page/film/moviedetail.jsp">ID : damm</a>
				</h6>
			</header>
		</article>

		<article>
			<a href="#" class="image featured"><img
				src="/MovieHolic/images/pic04.jpg" alt="" /></a>
			<header>
				<h6>
					키트리추천영화<br>
					<a href="/MovieHolic/page/film/moviedetail.jsp">ID : damm</a>
				</h6>
			</header>
		</article>

		<article>
			<a href="#" class="image featured"><img
				src="/MovieHolic/images/pic05.jpg" alt="" /></a>
			<header>
				<h6>
					키트리추천영화<br>
					<a href="/MovieHolic/page/film/moviedetail.jsp">ID : damm</a>
				</h6>
			</header>
		</article>

	</div>
</section>

<!-- user movie list -->
<section class="carousel" style="padding:0 0 0 0;">
	<div class="reel">
		<article>
			<a href="#" class="image featured"><img
				src="/MovieHolic/images/brother.jpg" alt="" /></a>
			<header>
				<h6>
					키트리추천영화<br>
					<a href="/MovieHolic/page/film/moviedetail.jsp">ID : damm</a>
				</h6>
			</header>
		</article>

		<article>
			<a href="#" class="image featured"><img
				src="/MovieHolic/images/avengers4.jpg" alt="" /></a>
			<header>
				<h6>
					키트리추천영화<br>
					<a href="/MovieHolic/page/film/moviedetail.jsp">ID : damm</a>
				</h6>
			</header>
		</article>

		<article>
			<a href="#" class="image featured"><img
				src="/MovieHolic/images/capma.jpg" alt="" /></a>
			<header>
				<h6>
					키트리추천영화<br>
					<a href="/MovieHolic/page/film/moviedetail.jsp">ID : damm</a>
				</h6>
			</header>
		</article>

		<article>
			<a href="#" class="image featured"><img
				src="/MovieHolic/images/watching.jpg" alt="" /></a>
			<header>
				<h6>
					키트리추천영화<br>
					<a href="/MovieHolic/page/film/moviedetail.jsp">ID : damm</a>
				</h6>
			</header>
		</article>

		<article>
			<a href="#" class="image featured"><img
				src="/MovieHolic/images/money.jpg" alt="" /></a>
			<header>
				<h6>
					키트리추천영화<br>
					<a href="/MovieHolic/page/film/moviedetail.jsp">ID : damm</a>
				</h6>
			</header>
		</article>

		<article>
			<a href="#" class="image featured"><img
				src="/MovieHolic/images/pic01.jpg" alt="" /></a>
			<header>
				<h6>
					키트리추천영화<br>
					<a href="/MovieHolic/page/film/moviedetail.jsp">ID : damm</a>
				</h6>
			</header>
		</article>

		<article>
			<a href="#" class="image featured"><img
				src="/MovieHolic/images/pic02.jpg" alt="" /></a>
			<header>
				<h6>
					범준쓰추천영화<br>
					<a href="/MovieHolic/page/film/moviedetail.jsp">ID : beomjj</a>
				</h6>
			</header>
		</article>

		<article>
			<a href="#" class="image featured"><img
				src="/MovieHolic/images/pic03.jpg" alt="" /></a>
			<header>
				<h6>
					나의 인생영화<br>
					<a href="/MovieHolic/page/film/moviedetail.jsp">ID : gwangq</a>
				</h6>
			</header>
		</article>

		<article>
			<a href="#" class="image featured"><img
				src="/MovieHolic/images/pic04.jpg" alt="" /></a>
			<header>
				<h6>
					키트리추천영화<br>
					<a href="/MovieHolic/page/film/moviedetail.jsp">ID : dam</a>
				</h6>
			</header>
		</article>

		<article>
			<a href="#" class="image featured"><img
				src="/MovieHolic/images/pic05.jpg" alt="" /></a>
			<header>
				<h6>
					키트리추천영화<br>
					<a href="/MovieHolic/page/film/moviedetail.jsp">ID : shz</a>
				</h6>
			</header>
		</article>

	</div>
</section>
<hr />
<div class="row">
	<article class="col-4 col-12-mobile special">
		<a href="#" class="image featured"><img src="images/pic07.jpg"
			alt="" /></a>
		<header>
			<h3>
				<a href="#">Gravida aliquam penatibus</a>
			</h3>
		</header>
		<p>Amet nullam fringilla nibh nulla convallis tique ante proin
			sociis accumsan lobortis. Auctor etiam porttitor phasellus tempus
			cubilia ultrices tempor sagittis. Nisl fermentum consequat integer
			interdum.</p>
	</article>
	<article class="col-4 col-12-mobile special">
		<a href="#" class="image featured"><img src="images/pic08.jpg"
			alt="" /></a>
		<header>
			<h3>
				<a href="#">Sed quis rhoncus placerat</a>
			</h3>
		</header>
		<p>Amet nullam fringilla nibh nulla convallis tique ante proin
			sociis accumsan lobortis. Auctor etiam porttitor phasellus tempus
			cubilia ultrices tempor sagittis. Nisl fermentum consequat integer
			interdum.</p>
	</article>
	<article class="col-4 col-12-mobile special">
		<a href="#" class="image featured"><img src="images/pic09.jpg"
			alt="" /></a>
		<header>
			<h3>
				<a href="#">Magna laoreet et aliquam</a>
			</h3>
		</header>
		<p>Amet nullam fringilla nibh nulla convallis tique ante proin
			sociis accumsan lobortis. Auctor etiam porttitor phasellus tempus
			cubilia ultrices tempor sagittis. Nisl fermentum consequat integer
			interdum.</p>
	</article>
</div>
<%@ include file="/template/footer.jsp"%>